...
// Context-sensitive: detects when the selection is a list (offers
// convert-to-tasks / bullets / numbered) and otherwise shows inline
// formatting actions.
// ============================================================
...
export function setupFormatToolbar() {
  tb = $('formatToolbar');
  if (!tb) return;
  // Don't lose selection while clicking toolbar buttons.
  tb.addEventListener('mousedown', (e) => e.preventDefault());
  tb.addEventListener('click', (e) => {
    const btn = e.target.closest('button[data-fmt]');
    if (btn) { applyFormat(btn.dataset.fmt); refresh(); }
  });
  window.addEventListener('yanta-selection-change', refresh);
  document.addEventListener('selectionchange', refresh);
  window.addEventListener('scroll', () => { if (!tb.hidden) refresh(); }, true);
}
...
function refresh() {
  ...
  // Build button set based on selection context.
  const text = v.state.sliceDoc(sel.from, sel.to);
  const lines = text.split('\n');
  const allTasks = lines.length > 0 && lines.every((l) => /^\s*[-*+]\s+\[[ xX]\]/.test(l) || l.trim() === '');
  const allBullets = lines.length > 0 && lines.every((l) => /^\s*[-*+]\s+/.test(l) || l.trim() === '');
  const allOrdered = lines.length > 0 && lines.every((l) => /^\s*\d+\.\s+/.test(l) || l.trim() === '');
  const isMultiline = lines.length > 1;
  const html = [];
  const btn = (fmt, label, title) => `<button data-fmt="${fmt}" title="${title || label}">${label}</button>`;
  if (allTasks) {
    ...
    html.push(btn('bold', 'B', 'Bold (Ctrl+B)'));
    html.push(btn('italic', 'I', 'Italic (Ctrl+I)'));
  ...
  if (!coordsFrom || !coordsTo) { hide(); return; }
...
function hide() { if (tb) tb.hidden = true; }
...
function applyFormat(fmt) {
  ...
  if (wraps[fmt]) {
    const text = v.state.sliceDoc(sel.from, sel.to);
    const open = wraps[fmt], close = wraps[fmt];
    v.dispatch({
      changes: { from: sel.from, to: sel.to, insert: open + text + close },
      selection: { anchor: sel.from + open.length + text.length + close.length },
    });
    return;
  }
  if (fmt === 'link') {
  ...
  // Line-prefix or block conversions: operate on whole lines in selection.
...
function toggleTaskLine(line) {
  return line.replace(/(^\s*[-*+]\s+\[)([ xX])(\])/, (_, a, c, d) => a + (c.toLowerCase() === 'x' ? ' ' : 'x') + d);
}
