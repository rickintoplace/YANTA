// ============================================================
// YANTA — Interactive force-directed graph view (canvas).
// Nodes = notes, edges = wikilinks. Pan/zoom/drag/search.
// ============================================================
...
const graph = {
  ...
  hover: null, highlight: '', running: false,
};

function buildGraph() {
  graph.nodes = []; graph.links = []; graph.idIndex.clear();
  ...
  for (const n of state.notes.values()) {
    ...
    graph.nodes.push({ id: n.id, title: n.title || 'Untitled', x: cx + Math.cos(angle) * r, y: cy + Math.sin(angle) * r, vx: 0, vy: 0, degree: 0 });
    i++;
  }
  for (const n of state.notes.values()) {
    let body = ''; try { body = noteMarkdown(n.id); } catch {}
    const seen = new Set();
...
function stepGraph() {
  const repulsion = 1200, attraction = 0.012, gravity = 0.006, damping = 0.82;
...
function drawGraph() {
  ...
  const accent = s.getPropertyValue('--accent').trim() || '#6ea8fe';
  const dim = s.getPropertyValue('--text-faint').trim() || '#5b6270';
  const border = s.getPropertyValue('--border').trim() || '#2a313c';
  const text = s.getPropertyValue('--text').trim() || '#d8dee9';
  ctx.lineWidth = 1 / graph.scale;
  ctx.strokeStyle = border;
  ctx.beginPath();
  for (const l of graph.links) { const a = graph.nodes[l.a], b = graph.nodes[l.b]; ctx.moveTo(a.x, a.y); ctx.lineTo(b.x, b.y); }
  ctx.stroke();
  ...
  for (const n of graph.nodes) {
    ...
    if (isHover) { ctx.shadowColor = accent; ctx.shadowBlur = 12; }
    ctx.fill();
    ctx.shadowBlur = 0;
    if (isCurrent) { ctx.strokeStyle = accent; ctx.lineWidth = 2 / graph.scale; ctx.stroke(); }
    if (graph.scale > 0.7 || isHover || isCurrent || matched) {
      ctx.fillStyle = text;
      ctx.font = (11 / graph.scale).toFixed(2) + 'px ' + s.fontFamily;
      ctx.textAlign = 'left';
...
function animate() { if (!graph.running) return; stepGraph(); drawGraph(); graph.raf = requestAnimationFrame(animate); }
...
function canvasCoords(e) {
  ...
  return { x: (e.clientX - r.left - graph.ox) / graph.scale, y: (e.clientY - r.top - graph.oy) / graph.scale };
}
...
export function setupGraphInteractions() {
  const c = $('graphCanvas');
  let pressMx = 0, pressMy = 0, moved = 0;
  c.addEventListener('mousedown', (e) => {
    pressMx = e.clientX; pressMy = e.clientY; moved = 0;
    const pos = canvasCoords(e);
    const hit = nodeAt(pos.x, pos.y);
    if (hit) { graph.dragNode = hit; graph.dragMx = pos.x - hit.x; graph.dragMy = pos.y - hit.y; }
    else { graph.panning = true; graph.dragMx = e.clientX; graph.dragMy = e.clientY; c.classList.add('dragging'); }
  });
  window.addEventListener('mousemove', (e) => {
    ...
    if (graph.dragNode) {
      ...
      graph.dragMx = e.clientX; graph.dragMy = e.clientY;
    } else {
      const pos = canvasCoords(e);
      graph.hover = nodeAt(pos.x, pos.y);
    }
  });
  window.addEventListener('mouseup', () => { graph.dragNode = null; graph.panning = false; if (graph.canvas) graph.canvas.classList.remove('dragging'); });
  c.addEventListener('click', (e) => {
    if (moved > 5 || graph.panning) return;
    const pos = canvasCoords(e);
    const hit = nodeAt(pos.x, pos.y);
  ...
  c.addEventListener('wheel', (e) => {
    ...
    const ns = Math.max(0.2, Math.min(4, graph.scale * factor));
  ...
  $('graphRecenter').addEventListener('click', () => { graph.scale = 1; graph.ox = 0; graph.oy = 0; });
